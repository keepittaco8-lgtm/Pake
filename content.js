...
(() => {
  ...
  function injectExportButtons() {
    ...
    const responses = document.querySelectorAll(
      "model-response:not([data-export-injected])",
    );
          ...
          if (annotation) {
            ...
            while (curr && curr !== node) {
              if (
                ...
                (curr.classList && curr.classList.contains("math-block"))
              ) {
                isBlock = true;
                break;
              }
  ...
  async function fetchRawMarkdown(element) {
    ...
    let container = element.closest(
      '[id^="r_"], [id^="c_"], div.conversation-container[id]',
    );
    if (!container) return "";
    ...
    return new Promise((resolve, reject) => {
      ...
      const listener = (event) => {
        if (
          event.source === window &&
          event.data &&
          event.data.type === "PAGE_FUNCTION_RESULT" &&
  ...
  async function extractChartsFromNode(node) {
    ...
    for (let i = 0; i < svgs.length; i++) {
      ...
      const height =
        svg.clientHeight ||
        (svg.getBoundingClientRect && svg.getBoundingClientRect().height) ||
        0;
      ...
      const isMermaid =
        ...
        (svg.id && svg.id.includes("mermaid"));
  ...
  async function exportToFormat(node, format) {
    ...
    try {
      ...
      if (!rawContent) {
        alert(
          "No content available to export. (Refresh the page and chat again so the extension can hook the data stream).",
        );
      ...
      if (blockData) {
        ...
        if (queryEl && queryEl.querySelector) {
          const userTextEl = queryEl.querySelector(
            '[data-mime-type="text/plain"], .query-text, .user-query-text',
          );
      ...
      const blob = new Blob([fullDocumentMarkdown], {
  ...
  function injectExportAllButton() {
    ...
    floatBtn.title =
      "Auto-scrolls to load all chat history and exports to Word";
      ...
      const interval = setInterval(() => {
        try {
          ...
          const allContainers = document.querySelectorAll(
            'message-list, main, [role="main"], div.conversation-container, div[style*="overflow"], infinite-scroller, .infinite-scroller',
          );
          ...
          setTimeout(() => {
            try {
              const allMsgs = document.querySelectorAll(
                "model-response, user-query, current-user-message, .conversation-container",
              );
              ...
              if (allMsgs.length > 0) {
                currentMetric +=
                  "_topContent:" +
                  allMsgs[0].textContent.substring(0, 80).trim();
              }

              if (currentMetric === lastMetric) {
                unchangedCount++;
                if (unchangedCount > 5) {
                  // Wait roughly 5 seconds of NO new messages added
                  clearInterval(interval);
                  resolve();
                }
              } else {
  ...
  async function exportAllToWord() {
    ...
    btn.title =
      "Đang cuộn lên trên cùng... Click để dừng và tải xuống dữ liệu hiện tại ngay lập tức.";
